...
await page.waitForTimeout(500);
await page.click('.btn.primary:has-text("选择位置并下载")');
...
check('图片预览已生成', imgOk);
if (imgOk) {
  const dlImg = page.waitForEvent('download', { timeout: 20000 }).catch(() => null);
  await page.click('.btn.primary:has-text("选择位置并下载")');
  const im = await dlImg;
  check('图片导出触发下载', !!im, im?.suggestedFilename());
} else {
  check('图片导出触发下载', false, '预览未生成');
}
await page.keyboard.press('Escape').catch(() => {});
await page.waitForTimeout(400);
await page.click('.modal .btn:has-text("关闭")').catch(() => {});
