...
const sw = await page.evaluate(() => {
  ...
  return btns.map((b) => ({ bg: getComputedStyle(b).backgroundColor, on: b.className.includes('on'), style: b.getAttribute('style') }));
});
console.log('色块列表:');
console.log(JSON.stringify(sw, null, 1));
// 点第一个色块（红色系 rgb(214,69,65)）
await page.evaluate(() => {
...
const after = await page.evaluate(() => {
  ...
  const pseudo = getComputedStyle(on, '::after');
  return { bg: cs.backgroundColor, border: cs.borderColor, transform: cs.transform, shadow: cs.boxShadow.slice(0, 100), afterBg: pseudo.backgroundColor, afterH: pseudo.height, style: on.getAttribute('style') };
});
console.log('点红块后选中:', JSON.stringify(after, null, 1));
const ok = after && after.bg === 'rgb(225, 112, 85)' && after.border === 'rgb(108, 92, 231)' && after.afterBg === 'rgb(108, 92, 231)' && parseFloat(after.afterH) === 3;
console.log(ok ? '✅ 选中标记移到所点色块+紫色底部指示条(zc风格)' : '❌ ' + JSON.stringify(after));
await browser.close();
process.exit(0);
