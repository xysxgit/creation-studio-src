...
const before = await page.evaluate(() => {
  return [...document.querySelectorAll('.ctx-menu .palette-row button:not(.on)')].slice(0, 3).map((b) => {
    const cs = getComputedStyle(b);
    return { inlineStyle: b.getAttribute('style'), bg: cs.backgroundColor };
  });
});
console.log('未选中色块:', JSON.stringify(before));
// 点青色 #00cec9（slice(0,10) 第9个 = 索引8）
await page.evaluate(() => {
  const bs = [...document.querySelectorAll('.ctx-menu .palette-row button')];
  const t = bs.find((b) => (b.getAttribute('style') || '').includes('0, 206, 201'));
  if (t) t.click();
});
...
const after = await page.evaluate(() => {
  ...
  return { inlineStyle: on.getAttribute('style'), bg: cs.backgroundColor, shadow: cs.boxShadow.slice(0, 60) };
});
console.log('选中青色块:', JSON.stringify(after));
console.log('青色保持:', after && after.bg === 'rgb(0, 206, 201)' ? '✅' : '❌ 白=' + after?.bg);
await browser.close();
process.exit(0);